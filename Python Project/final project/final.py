...
class DataAnalysisVisualizer:
    ...
    def data_visualization(self):
        ...
        print("4. pie Chart")
        print("5. Legend Outside")

        choice = self._input_int("Enter your choice: ", 1, 6)
        ...
        if choice == 1:
            ...
            if not (
                pd.api.types.is_numeric_dtype(self.df[x_col]) and
                pd.api.types.is_numeric_dtype(self.df[y_col])
            ):
                print("Comparison graph requires numeric columns!")
                return

            fig, ax = plt.subplots()

            ax.plot(self.df[x_col], label=x_col)
            ax.plot(self.df[y_col], label=y_col)

            ax.set_xlabel("Index")
            ax.set_ylabel("Values")
            ax.set_title(f"Comparison of {x_col} and {y_col}")
